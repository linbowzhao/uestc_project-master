    ...
    <div class="row">
        <div class="col-md-7">
            ...
            if(isset($_GET['i'])&& $_GET['i']==1){
                $total = mysqli_fetch_array(mysqli_query("select count(*) from passage where importance=1"));//查询数据库中精选一共有多少条数据
            }else{
                $total = mysqli_fetch_array(mysqli_query("select count(*) from passage"));//查询数据库中一共有多少条数据
            }
			$Total = $total[0];//    
